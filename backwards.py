...
if __name__ == '__main__':
    ...
    (x,y) = (0,0)
    ansFile = open('outputBackwards.txt', 'w')
    lengthx, lengthy = (10,10)
    while(x != lengthx or y != lengthy):
        for row in arr:
            ansFile.writelines(row)
            ansFile.write("\n")
        ansFile.write("\n")
        finalNode = astar(lengthx,lengthy,currentx,currenty,exploredList)
        if(finalNode == None):
        ...
        path = {}
        while(finalNode):
            path[(finalNode.xcoord, finalNode.ycoord)] = finalNode
